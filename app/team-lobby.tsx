  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        <LinearGradient colors={['#ad46ff', '#4f39f6']} style={styles.teamCard}>
            ...
            {teamMembers.map((member, index) => (
              <View key={index} style={styles.memberItem}>
                <View style={styles.memberLeft}>
                  <Avatar initial={member.initial} size={32} backgroundColor="#ffffff4c" borderColor="#ffffff" borderWidth={2} />
                  <Text style={styles.memberName}>{member.name}</Text>
                </View>
                <Badge text={member.status} backgroundColor={member.ready ? '#00c950' : '#ffffff33'} textColor="#ffffff" />
              </View>
            ))}
          ...
          <Button title="クイズを開始" onPress={() => router.push('/quiz')} style={styles.startButton} textStyle={styles.startButtonText} />
        </LinearGradient>
        ...
        <TouchableOpacity>
          <LinearGradient colors={['#10b981', '#14b8a6']} style={styles.createTeamButton}>
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingTop: 60, paddingHorizontal: 20, paddingBottom: 12, backgroundColor: '#ffffff', borderBottomWidth: 1, borderBottomColor: '#e1e8f0' },
  backIcon: { fontSize: 24, color: '#0e162b' },
  headerTitle: { fontSize: 16, color: '#0e162b', fontWeight: '400', letterSpacing: -0.31 },
  placeholder: { width: 24 },
  content: { flex: 1, padding: 20 },
  teamCard: { borderRadius: 14, padding: 20, gap: 20, marginBottom: 20 },
  ...
  teamIcon: { fontSize: 16 },
  teamName: { fontSize: 16, color: '#ffffff', fontWeight: '400', letterSpacing: -0.31 },
  membersList: { gap: 10 },
  memberItem: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#ffffff1a', padding: 10, borderRadius: 14 },
  memberLeft: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  memberName: { fontSize: 16, color: '#ffffff', fontWeight: '400', letterSpacing: -0.31 },
  inviteButton: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 6, backgroundColor: '#ffffff1a', padding: 10, borderRadius: 14 },
  inviteIcon: { fontSize: 16, color: '#ffffff' },
  inviteText: { fontSize: 16, color: '#ffffff', fontWeight: '400', letterSpacing: -0.31 },
  startButton: { backgroundColor: '#ffffff', height: 38 },
  startButtonText: { color: '#980ffa', fontSize: 14 },
  availableSection: { marginBottom: 20 },
  sectionTitle: { fontSize: 16, color: '#0e162b', fontWeight: '400', letterSpacing: -0.31, marginBottom: 12 },
  teamsList: { gap: 10 },
  ...
  onlineDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: '#00a63d' },
  statusText: { fontSize: 12, color: '#00a63d' },
  membersInfo: { alignItems: 'flex-end' },
  membersCount: { fontSize: 14, color: '#45556c', letterSpacing: -0.15 },
  membersLabel: { fontSize: 12, color: '#90a1b8' },
  joinButton: { backgroundColor: '#0f172b', height: 26 },
  joinButtonText: { color: '#ffffff', fontSize: 14 },
  createTeamButton: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 16, borderRadius: 14 },
  createTeamLeft: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  createTeamIcon: { fontSize: 32, color: '#ffffff' },
  createTeamTitle: { fontSize: 16, color: '#ffffff', fontWeight: '400', letterSpacing: -0.31 },
  createTeamSubtitle: { fontSize: 14, color: '#ffffff', opacity: 0.9, letterSpacing: -0.15 },
  arrowIcon: { fontSize: 16, color: '#ffffff' },
});
